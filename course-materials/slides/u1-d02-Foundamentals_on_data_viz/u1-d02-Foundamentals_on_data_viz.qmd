---
title: "Fundamentos en Visualización de Datos"
subtitle: "Módulo V - Analítica y Visualización de Datos, Aplicaciones"

author: "Rolando Mirabal"
format: 
  revealjs:
    theme: simple
    transition: fade
    slide-number: true
    chalkboard: true
editor: visual
---

## Video 1 - La alegría de la Estadística

{{< video https://youtu.be/V8lbiiTF2P0 width="100%" height="85%" >}}

# 1. ¿Por qué visualizamos datos?

## 1.1. El cuarteto de Frank Anscombe {.smaller .scrollable transition="slide"}

```{r}
library(dplyr)
anscombe_m <- tibble(a.x = c(10, 8, 13, 9, 11, 14, 6, 4, 12, 7, 5),
                    a.y = c(8.04,6.95,7.58,8.81,8.33,9.96,7.24,4.26,10.84,4.82,5.68),
                    b.x = c(10, 8, 13, 9, 11, 14, 6, 4, 12, 7, 5),
                    b.y = c(9.14, 8.14, 8.74, 8.77, 9.26,8.10, 6.13, 3.10, 9.13, 7.26, 4.74),
                    c.x = c(10, 8, 13, 9, 11, 14, 6, 4, 12, 7, 5),
                    c.y = c(7.46, 6.77, 12.74, 7.11, 7.81, 8.84, 6.08, 5.39, 8.15, 6.42, 5.73),
                    d.x = c(8,8,8,8,8,8,8,19,8,8,8),
                    d.y = c(6.58,5.76,7.71,8.84,8.47,7.04,5.25,12.5, 5.56,7.91,6.89))
                    

knitr::kable(anscombe_m, digits = 2) %>% 
  kableExtra::add_header_above(c("Grupo A" = 2, "Grupo B" = 2, "Grupo C" = 2, "Grupo D" = 2))
```

::: notes
Tabla con cuatro grupos de números: ¿Qué te dicen?
¿Qué puedes ver?
¿Hay algúnas diferencias discernibles entre ellas en los patrones o tendencias?
:::

## ¿Estadísticamente iguales? {.smaller .scrollable transition="slide"}

```{r}
knitr::kable(summary(anscombe_m))
```

## Graficando el Cuarteto de Frank Anscombe

```{r}
library(tidyverse)
anscombe_tidy <- anscombe %>%
    mutate(observation = seq_len(n())) %>%
    gather(key, value, -observation) %>%
    separate(key, c("variable", "set"), 1, convert = TRUE) %>%
    mutate(set = c("A", "B", "C", "D")[set]) %>%
    spread(variable, value)

ggplot(anscombe_tidy, aes(x, y)) +
    geom_point() +
    facet_wrap(~ set) +
    geom_smooth(method = "lm", se = FALSE)

```
::: notes
ahora, ¿puedes ver la diferencia?
Ver los númberos en un gráfico muestra algo que las tablas y alguna medidas estadísticas no pueden.
Visualizamos los datos para aprovechas el poder increible de nuestro sistema visual para encontrar relaciones y tendencias
:::

# 2. ¿Cómo visualizamos los Datos?

## 2.1. Atributos preatentivos

## 2.2. Tipos de datos

# 3. ¿Qué es Visualización de Datos?

## ¿Qué es Analítica de Datos?

-   Cuando las técnicas de visualización contribuyen y ayudan al de visualizaciones éxitosas.

-   
